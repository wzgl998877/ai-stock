...
与原计划相比，主要差异在于：
- 模块三从"缠论策略监控"变更为"数据与工具"面板；缠论策略监控与历史回测已独立实现（spec 009，见 `strategy-monitor-prd-v2.md` / `backtest-prd-v2.md`）
...
| 批量行情 | 无 | 批量获取多只股票实时行情 |
| 模块三定位 | 缠论策略监控 | 数据与工具面板（缠论监控/回测见 spec 009） |
...
| 功能 | 说明 |
|------|------|
| 缠论策略监控 | 已独立实现（spec 009），归属策略监控模块，非本模块（行情数据）范围 |
| K线画线工具 | 用户明确不需要 |
| Level 2 数据 | 超出免费数据范围 |
| 股票财务深度分析 | 用户财务知识有限 |
| 港股/美股 | 只做 A 股 |
